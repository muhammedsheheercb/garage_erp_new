...
export function JobCardPrintClient({ job }: { job: any }) {
  ...
  const items = [
    ...
    ...(job.parts || []).map((item: any) => ({ type: "Part / قطعة", name: `${item.batch?.inventory?.itemName || ""}${item.batch?.inventory?.partNumber ? ` (${item.batch.inventory.partNumber})` : ""}`, quantity: item.quantity || 1, price: item.price || 0, id: `part-${item.id}` })),
  ];
