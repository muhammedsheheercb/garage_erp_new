...
export function CompanyList() {
  ...
  return <div className="space-y-4">
    ...
    {isLoading ? <p className="py-12 text-center text-muted-foreground">{t.common.loading}</p> : companies.length === 0 ? <Card><CardContent className="py-12 text-center text-muted-foreground">{t.common.noResults}</CardContent></Card> : <div className="grid gap-4 md:grid-cols-2">
      {companies.slice((companyPage - 1) * 5, companyPage * 5).map((company) => <Card key={company.id}>
        ...
        <CardContent className="space-y-2"><div className="flex justify-between items-center"><p className="text-sm font-medium">{t.vehicles.model}</p><Dialog open={addingModelFor?.id === company.id} onOpenChange={(open) => !open && setAddingModelFor(null)}><DialogTrigger render={<Button size="sm" variant="outline" onClick={() => setAddingModelFor(company)}><Plus className="mr-1 h-3.5 w-3.5" />{t.vehicles.addModel}</Button>} />{addingModelFor?.id === company.id && <DialogContent><DialogHeader><DialogTitle>{t.vehicles.addModel}</DialogTitle></DialogHeader><ModelForm companyId={company.id} onSuccess={() => setAddingModelFor(null)} /></DialogContent>}</Dialog></div>
          {company.models.length === 0 ? <p className="text-sm text-muted-foreground">{t.vehicles.noModels}</p> : <div className="divide-y rounded-md border">{company.models.slice(((modelPages[company.id] || 1) - 1) * 5, (modelPages[company.id] || 1) * 5).map((model) => <div key={model.id} className="flex items-center justify-between px-3 py-2 text-sm"><span>{model.name}</span><div className="flex gap-1"><Dialog open={editingModel?.model.id === model.id} onOpenChange={(open) => !open && setEditingModel(null)}><DialogTrigger render={<Button size="icon" variant="ghost" onClick={() => setEditingModel({ companyId: company.id, model })}><Edit className="h-3.5 w-3.5" /></Button>} />{editingModel?.model.id === model.id && <DialogContent><DialogHeader><DialogTitle>{t.vehicles.editModel}</DialogTitle></DialogHeader><ModelForm companyId={company.id} model={model} onSuccess={() => setEditingModel(null)} /></DialogContent>}</Dialog><AlertDialog><AlertDialogTrigger render={<Button size="icon" variant="ghost" className="text-destructive"><Trash className="h-3.5 w-3.5" /></Button>} /><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>{t.vehicles.deleteModel}</AlertDialogTitle><AlertDialogDescription>{t.vehicles.deleteConfirm}</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>{t.common.cancel}</AlertDialogCancel><AlertDialogAction onClick={() => modelDelete.mutate(model.id)} className="bg-destructive text-destructive-foreground">{t.common.delete}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog></div></div>)}</div>}
          {company.models.length > 5 && <div className="flex items-center justify-end gap-2 pt-2 text-sm"><Button variant="outline" size="sm" onClick={() => setModelPages((pages) => ({ ...pages, [company.id]: Math.max(1, (pages[company.id] || 1) - 1) }))} disabled={(modelPages[company.id] || 1) === 1}><ChevronLeft className="h-4 w-4" /></Button><span className="text-muted-foreground">{modelPages[company.id] || 1} / {Math.ceil(company.models.length / 5)}</span><Button variant="outline" size="sm" onClick={() => setModelPages((pages) => ({ ...pages, [company.id]: Math.min(Math.ceil(company.models.length / 5), (pages[company.id] || 1) + 1) }))} disabled={(modelPages[company.id] || 1) === Math.ceil(company.models.length / 5)}><ChevronRight className="h-4 w-4" /></Button></div>}
        </CardContent>
      </Card>)}
    </div>}
    {companies.length > 5 && <div className="flex items-center justify-end gap-2 text-sm"><Button variant="outline" size="sm" onClick={() => setCompanyPage((page) => Math.max(1, page - 1))} disabled={companyPage === 1}><ChevronLeft className="h-4 w-4" /></Button><span className="text-muted-foreground">{companyPage} / {Math.ceil(companies.length / 5)}</span><Button variant="outline" size="sm" onClick={() => setCompanyPage((page) => Math.min(Math.ceil(companies.length / 5), page + 1))} disabled={companyPage === Math.ceil(companies.length / 5)}><ChevronRight className="h-4 w-4" /></Button></div>}
  </div>
}
