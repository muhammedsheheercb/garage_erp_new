...
export function InventoryForm({ initialData, onSuccess }: InventoryFormProps) {
  ...
  const { register, handleSubmit, control, setValue, formState: { errors } } = useForm<InventoryFormValues>({
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          ...
          <Input id="partNumber" placeholder="Generating..." readOnly className="bg-muted" {...register("partNumber")} />
