...
type Row = { batchId: string; label: string; available: number; quantity: number; purchasePrice: number; salesPrice: number }
...
export function DirectSaleForm({ initialData, onSuccess }: { initialData?: any; onSuccess: () => void }) {
  ...
  const [rows, setRows] = useState<Row[]>(() => initialData?.items?.map((item: any) => ({ batchId: item.batchId, label: `${item.batch.inventory.itemName} — Batch ${item.batch.batchNumber}`, available: item.quantity, quantity: item.quantity, purchasePrice: item.purchasePrice, salesPrice: item.salesPrice })) || [])
  const [discount, setDiscount] = useState(initialData?.discount || 0)
  const [errors, setErrors] = useState<Errors>({})
  const subtotal = rows.reduce((sum, row) => sum + row.quantity * row.salesPrice, 0)
  const vat = rows.reduce((sum, row) => sum + row.quantity * row.salesPrice * .05, 0)
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      const data = { ...customer, saleDate, discount: billDiscount, items: rows.map(row => ({ batchId: row.batchId, quantity: row.quantity, purchasePrice: row.purchasePrice, salesPrice: row.salesPrice, vat: 5 })) }
  ...
  const addPart = (part: any) => setRows(current => [...current, { batchId: part.id, label: `${part.itemName} — Batch ${part.batchNumber}`, available: part.availableQuantity, quantity: 1, purchasePrice: part.purchasePrice, salesPrice: part.purchasePrice }])
...
function ProductRows({ rows, updateRow, removeRow }: { rows: Row[]; updateRow: (index: number, patch: Partial<Row>) => void; removeRow: (index: number) => void }) {
  return <><div className="space-y-3 md:hidden">{rows.map((row, index) => <ProductCard key={row.batchId} row={row} index={index} updateRow={updateRow} remove={() => removeRow(index)} />)}</div><div className="hidden overflow-x-auto rounded-lg border md:block"><table className="w-full min-w-[900px] text-sm"><thead className="bg-muted"><tr><th className="p-3 text-left">Part / Batch</th><th>Available</th><th>Qty</th><th>Purchase Price</th><th>Sales Price</th><th>VAT %</th><th>VAT Amount</th><th>Total Amount</th><th /></tr></thead><tbody>{rows.map((row, index) => { const tax = row.quantity * row.salesPrice * .05; return <tr className="border-t" key={row.batchId}><td className="p-3 font-medium">{row.label}</td><td className="text-center">{row.available}</td><td className="p-2"><Input type="number" min="1" max={row.available} value={row.quantity} onChange={event => updateRow(index, { quantity: Number(event.target.value) || 1 })} /></td><td className="p-2"><Input readOnly value={row.purchasePrice} /></td><td className="p-2"><Input type="number" min="0" step="any" value={row.salesPrice} onChange={event => updateRow(index, { salesPrice: Math.max(0, Number(event.target.value)) })} /></td><td className="text-center">5%</td><td className="text-right">{money(tax)}</td><td className="pr-3 text-right font-medium">{money(row.quantity * row.salesPrice + tax)}</td><td><Button type="button" variant="ghost" size="icon" onClick={() => removeRow(index)}><Trash className="h-4 w-4 text-destructive" /></Button></td></tr> })}</tbody></table></div></>
}

function ProductCard({ row, index, updateRow, remove }: { row: Row; index: number; updateRow: (index: number, patch: Partial<Row>) => void; remove: () => void }) { const tax = row.quantity * row.salesPrice * .05; return <div className="space-y-3 rounded-lg border p-3"><div className="flex items-start justify-between gap-2"><div><p className="font-medium">{row.label}</p><p className="text-xs text-muted-foreground">Available in this batch: {row.available}</p></div><Button type="button" variant="ghost" size="icon" onClick={remove}><Trash className="h-4 w-4 text-destructive" /></Button></div><div className="grid grid-cols-2 gap-3"><MiniField label="Quantity"><Input type="number" min="1" max={row.available} value={row.quantity} onChange={event => updateRow(index, { quantity: Number(event.target.value) || 1 })} /></MiniField><MiniField label="Purchase Price"><Input readOnly value={row.purchasePrice} /></MiniField><MiniField label="Sales Price"><Input type="number" min="0" step="any" value={row.salesPrice} onChange={event => updateRow(index, { salesPrice: Math.max(0, Number(event.target.value)) })} /></MiniField><MiniField label="VAT"><Input readOnly value={`5% (${money(tax)})`} /></MiniField><MiniField label="Total Amount"><Input readOnly value={money(row.quantity * row.salesPrice + tax)} /></MiniField></div></div> }
