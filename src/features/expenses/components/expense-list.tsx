...
import { getExpenses, deleteExpense, getMonthlyExpenseReport } from "../actions"
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table"
import { Button } from "@/components/ui/button"
import { Input } from "@/components/ui/input"
import { format } from "date-fns"
import { Pencil, Trash2, Plus, Search, Calendar as CalendarIcon, Loader2 } from "lucide-react"
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
          <DialogTrigger render={<Button><Plus className="mr-2 h-4 w-4" /> Add Expense</Button>} />
          ...
          <Card>
            <CardHeader className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-2 border-b">
              ...
              <div className="flex gap-2 w-full sm:w-auto">
                <Select value={reportMonth.toString()} onValueChange={(v) => setReportMonth(parseInt(v as string))}>
                ...
                <Select value={reportYear.toString()} onValueChange={(v) => setReportYear(parseInt(v as string))}>
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <div className="text-4xl font-bold text-destructive">
                        {reportData?.total.toFixed(3) || "0.000"} <span className="text-lg font-medium text-muted-foreground">OMR</span>
                      </div>
                    </div>

                    <div className="space-y-3">
                      <h3 className="font-semibold border-b pb-2">By Category</h3>
                      {Object.keys(reportData?.categoryTotals || {}).length === 0 ? (
                        ...
                        <div className="space-y-3">
                          {Object.entries(reportData?.categoryTotals || {})
                            ...
                            .map(([category, amount]) => {
                              const percentage = reportData?.total ? ((amount as number) / reportData.total) * 100 : 0;
                              return (
                                <div key={category} className="space-y-1">
                                  <div className="flex justify-between text-sm">
                                    <span className="font-medium">{category}</span>
                                    <span className="font-semibold">{(amount as number).toFixed(3)} OMR</span>
                                  </div>
                     ...
                     <div className="space-y-4 max-h-[350px] overflow-y-auto pr-2">
                       {reportData?.expenses.length === 0 ? (
                         ...
                         reportData?.expenses.map(expense => (
                           <div key={expense.id} className="flex justify-between items-start border-b pb-3 last:border-0 last:pb-0">
                             <div>
                               <div className="font-medium text-sm">{expense.category}</div>
                               <div className="text-xs text-muted-foreground">
                                 {format(new Date(expense.date), "dd MMM")} {expense.description && `• ${expense.description}`}
                               </div>
                             </div>
